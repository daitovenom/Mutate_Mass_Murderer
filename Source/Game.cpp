...
bool Game::Init() {
	...
	//ロード
	MovieGraphHandle = LoadGraph("Assets/180half_f.gif");  //ムービー
	taitolGraphHandle = LoadGraph("321000000_mmw227fw.gif");  //タイトルで出てくるムービー
	sinnzouGraphHandle = LoadGraph("Assets/original.gif");  //敵がダメージを受けた時の心臓のムービー
	snowGraphHandle = LoadGraph("Assets/20160103081928.gif");  //雪ムービー
	anarogunoizuGraphHandle = LoadGraph("Assets/MOSHED-2020-6-1-23-33-37.gif");  //アナログノイズムービ
	...
	PlayMovieToGraph(MovieGraphHandle);  // ムービーを再生状態にします
	PlayMovieToGraph(taitolGraphHandle);  // タイトルで出てくるムービーを再生状態にします
	PlayMovieToGraph(sinnzouGraphHandle);  // 敵がダメージを受けた時の心臓のムービーを再生状態にします
	PlayMovieToGraph(snowGraphHandle);  // 雪ムービーを再生状態にします
	PlayMovieToGraph(anarogunoizuGraphHandle);  // アナログノイズムービーを再生状態にします
