		...
		if (Gunkirikae == 0) {
			if (Gfunction_status == 0) {
				...
				if (jyuuosenai == 0) {
					...
					//赤い敵のYに関係なく並び順がそろえばこのプログラムが実行される。
						//★位置の状態はプログラムは把握できるが主人公の向きまでは把握できない左向きのこの並びかもしれないため向きの情報はif文書く必要がある。
						//Y軸は敵1にして、
						//X軸が主人公＜敵1＜敵2の順に書かれている。
						if (migimuki == 1 &&
							pos[enemyY1][enemyX1][1] + 20 + playerattackhanndou3 >= pos[playerY][playerX][1] + nanameidouY + nanameidouY2 + nanameidouY3 &&
							pos[enemyY1][enemyX1][1] - 20 + playerattackhanndou3 <= pos[playerY][playerX][1] + nanameidouY + nanameidouY2 + nanameidouY3
						...
						/*//Y軸は敵1にして、そして、敵1のY軸と敵２のY軸が同じ時のプログラムでもある。
						//X軸が主人公＜敵1＜敵2の順に書かれている。
						else if (migimuki == 1 &&
							pos[enemyY1][enemyX1][1] + 20 + playerattackhanndou3 >= pos[playerY][playerX][1] + nanameidouY + nanameidouY2 + nanameidouY3 &&
							pos[enemyY1][enemyX1][1] - 20 + playerattackhanndou3 <= pos[playerY][playerX][1] + nanameidouY + nanameidouY2 + nanameidouY3 &&
						...
						//主人公が敵1を右に、敵2を左に囲まれたときのプログラム。
							//軸は敵1にして、★ここのプログラムが敵2をまたいで敵2に攻撃を可能にしたプログラム。
							// その他の敵1が右斜めや左した斜めにいても敵2を撃てると言った役割も持つだろう。
							//X軸が敵2＜主人公＜敵1の順に書かれている。
						else if (migimuki == 1
							&& pos[enemyY1][enemyX1][1] + 20 + playerattackhanndou3 >= pos[playerY][playerX][1] + nanameidouY + nanameidouY2 + nanameidouY3 &&
							pos[enemyY1][enemyX1][1] - 20 + playerattackhanndou3 <= pos[playerY][playerX][1] + nanameidouY + nanameidouY2 + nanameidouY3
							&& pos[enemyY1][enemyX1][0] + playerattackhanndou + playerattackhanndou2 + playerattackhanndou3 >= pos[playerY][playerX][0] + nanameidouX + migiidou + nanameidouX2 + nanameidouX3
						...
						//主人公＜敵2であるため敵１の軸にする必要がない。
						//Y軸は敵1
						//X軸が主人公＜敵2＜敵1の順に書かれている。
						/*else if (migimuki == 1 &&
						...
						//★★ここが敵1をまたいで敵2に攻撃できることを可能にしたプログラム。
						// その他の敵1が右斜めや左した斜めにいても敵2を撃てると言った役割も持つだろう。
						//Y軸は敵2
						//X軸が主人公＜敵2＜敵1の順に書かれている。
						else if (migimuki == 1 &&
						...
						//★★以下の二つのプログラムは敵1と敵2が同じY軸に居る時のための物である。もちろん上のように上のように
						//場合分けに敵1と敵2のY軸を混合した不等式を上のif文に加えれば良いが見にくくてめんどくさいので以下のように二つ作った。

						//★上のプログラムが必要なのはX座標において主人公、敵1、敵2の順番を考慮するため。
						//このプログラムは敵2の順番を気にせずに敵1だけを考慮すればよいため書いた。
						//以下は敵1を右向きから撃つためのプログラム。
							//Y軸は敵1にして、
							//X軸が主人公＜敵1の順に書かれている。//敵2がどこに居ても、上斜めや下斜めにいるような時での範囲プログラム。
						...
						//★上のプログラムが必要なのはX座標において主人公、敵1、敵2の順番を考慮するため。
						//このプログラムは敵1の順番を気にせずに敵2だけを考慮すればよいため書いた。
						// 
					//以下は敵2を右向きから撃つためのプログラム。
						//Y軸は敵2にして、
						//X軸が主人公＜敵2の順に書かれている。//敵１がどこに居ても、上斜めや下斜めにいるような時での範囲プログラム。
						else if (migimuki == 1
							&& pos[enemyY2][enemyX2][1] + 20 + playerattackhanndou3B >= pos[playerY][playerX][1] + nanameidouY + nanameidouY2 + nanameidouY3 &&
							pos[enemyY2][enemyX2][1] - 20 + playerattackhanndou3B <= pos[playerY][playerX][1] + nanameidouY + nanameidouY2 + nanameidouY3
						...
						//★↓ここからは左向きでの敵2のY軸と敵1のY軸の主人公が敵2にダメージを与える部分になる。
						//長いif文は敵1や敵2をまたいで攻撃してリ、貫通してふたりの敵に攻撃しないようにしたり、同じY軸にふたり敵がいる時に
						//主人公の向く方向の敵だけに攻撃を当てれるようにするためのプログラムである。
						...
						//★★以下の二つのif文は主人公と敵１、敵2と同じY軸にいるとして、敵2だけを攻撃するプログラム。
				//tyokusennkougeki = 3;とtyokusennkougeki = 4;のプログラムは主人公と敵1と敵2が同じY軸に居る時の銃の範囲のプログラム。
				//tyokusennkougeki = 3;は敵2に左向きで攻撃、tyokusennkougeki = 4は左向きに敵1を攻撃。
				//上二つに繋がるプログラムにも左向きの攻撃はあるが、ここはただの左向きの攻撃ではなく、
				// 
				// 以下は敵2と敵1を左向きから撃つためのプログラム。
				//Y軸の被った敵1と敵2を強調した部分なのだ。



						 //Y軸は敵2にして、
						//敵2＜主人公＜敵1の順に書かれているプログラム。
						...
						//★★主人公と敵１、敵2と同じY軸にいるとして、敵2だけを攻撃するプログラム。
						//tyokusennkougeki = 3;とtyokusennkougeki = 4;のプログラムは主人公と敵1と敵2が同じY軸に居る時の銃の範囲のプログラム。
						//tyokusennkougeki = 3;は敵2に左向きで攻撃、tyokusennkougeki = 4は左向きに敵1を攻撃。
						//上二つに繋がるプログラムにも左向きの攻撃はあるが、ここはただの左向きの攻撃ではなく、
						//Y軸の被った敵1と敵2を強調した部分なのだ。
						...
						//以下は敵1を右向きから撃つためのプログラム。
							//Y軸は敵1にして、
							//X軸が主人公>敵1の順に書かれている。//敵2がどこに居ても、上斜めや下斜めにいるような時でも攻撃可能な範囲プログラム。
						...
						//以下は敵1を右向きから撃つためのプログラム。
							//Y軸は敵2にして、
							//X軸が主人公>敵2の順に書かれている。//敵1がどこに居ても、上斜めや下斜めにいるような時で攻撃可能な範囲プログラム。
